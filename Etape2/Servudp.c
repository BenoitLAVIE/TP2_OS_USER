...
typedef struct { struct in_addr addr; char pseudo[64]; } Peer;
Peer table[MAX_PEERS];
int nb_peers = 0;
...
void ajouter_utilisateur(struct sockaddr_in *addr, char *pseudo) {
    ...
    if (nb_peers < MAX_PEERS) {
        table[nb_peers].addr = addr->sin_addr;
        strncpy(table[nb_peers].pseudo, pseudo, 63);
        nb_peers++;
